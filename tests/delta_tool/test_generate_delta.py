...
def test_generate_delta_happy_day(tmp_path, test_files):
    ...
    generate_delta(
        test_files["orig_fw"], test_files["new_fw"], delta_file, 512, 60
    )
    print(
        "Delta-file-size={delta} new-file-size={new}".format(
...
def test_cli_generate_delta_happy_day(tmp_path, test_files):
    ...
    cmd = [
        "--current-fw",
        test_files["orig_fw"].as_posix(),
        "--new-fw",
        test_files["new_fw"].as_posix(),
        "--output",
        delta_file.as_posix(),
        "--block-size",
        "1024",
